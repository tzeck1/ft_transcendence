...
class uWebSocketsTransport extends core.Transport {
    ...
    async onConnection(rawClient) {
        ...
        const sessionId = querystring__default['default'].parse(query).sessionId;
        ...
        this.app.post("/matchmake/*", (res, req) => {
            ...
            this.readJson(res, async (clientOptions) => {
                try {
                    ...
                    const response = await core.matchMaker.controller.invokeMethod(method, name, clientOptions);
                    if (!res.aborted) {
